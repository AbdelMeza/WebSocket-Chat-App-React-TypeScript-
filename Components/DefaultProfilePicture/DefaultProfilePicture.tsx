...
export function DefaultGroupProfile({ defaultColor, size }: { defaultColor: string | null, size: number }) {
    const sizeClass = p[`profileSize-${size}`]

    return <div className={`default-group-profile ${sizeClass}`} style={{ backgroundColor: `rgb(${defaultColor}, 0.2)`, borderColor: `rgb(${defaultColor}, 0.3)` }}>
        <svg xmlns="http://www.w3.org/2000/svg" width={25} viewBox="0 0 24 24" fill="currentColor" className="size-6" style={{ fill: `rgb(${defaultColor})` }}>
